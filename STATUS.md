...
Update [14.07.2026]: Bugfix nach manuellem Test von Schritt 5 (TowerBase.gd). Bug 2 (Turret stand bei nach links zeigenden Zielen kopfüber): Turret-Darstellung von reiner 360°-Rotation auf "Rotation + horizontale Spiegelung" umgestellt (Kingdom-Rush-Standardmuster). Neu _aim_turret(): Ziel rechts der Vertikalachse (abs(target_angle) <= PI/2) -> flip_h = false, Rotation = target_angle wie bisher; Ziel links -> flip_h = true, Rotation = wrapf(target_angle - PI, -PI, PI). Verifizierte Godot-Vorzeichen-Konvention im Code dokumentiert (die naheliegende Formel PI - target_angle wäre falsch gewesen). _is_turret_aligned() prüft jetzt die tatsächliche Barrel-Richtung (global_rotation + PI bei flip_h), sonst hätte ein gespiegelter Turret nie gefeuert. Bug 1 (Mündungsfeuer daneben) war Folgefehler: get_muzzle_global_position() spiegelt jetzt den lokalen MuzzlePoint-Offset mit, wenn flip_h aktiv ist (flip_h spiegelt nur die Textur, nicht die Sibling-Node-Position), und transformiert über _turret.to_global(). MuzzlePoint-Offset (23.53,-2.73) und Spawn-Code (nutzt get_muzzle_global_position) waren korrekt und blieben unverändert. turret_rotation_speed/turret_alignment_tolerance_deg unverändert. Legacy-Türme (kein Turret, kein MuzzlePoint) durch Null-Guards nicht betroffen. Nicht angefasst: Projektil-Flug, Einschlag-Effekt/Schaden, Wellen-Ablauf. Offen: erneuter manueller Godot-Test (Mündungsfeuer an der Mündung auch nach links/oben, Turret nie kopfüber, IDLE-Verhalten) - kann nicht headless geprüft werden.
